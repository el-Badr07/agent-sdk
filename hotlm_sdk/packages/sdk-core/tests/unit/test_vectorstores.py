...
class SimpleEmbeddings(BaseEmbeddings):
    """Simple deterministic embeddings for testing."""
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        """Create simple embeddings based on text length."""
        return [[len(text) / 100, 0.5, 0.5, 0.5] for text in texts]
    
    def embed_query(self, text: str) -> List[float]:
        """Create simple query embedding based on text length."""
        return [len(text) / 100, 0.5, 0.5, 0.5]
...
def test_faiss_vectorstore_add_documents():
    """Test adding documents to FAISS vector store."""
    embeddings = SimpleEmbeddings()
    vectorstore = FaissVectorStore(embeddings=embeddings, dimension=4)
    
    docs = [
        Document(page_content="short", metadata={"key": "a"}),
        Document(page_content="medium length", metadata={"key": "b"}),
        Document(page_content="this is the longest document", metadata={"key": "c"})
    ]
...
def test_faiss_vectorstore_similarity_search():
    ...
    # Search for a short query (should match the shortest document best)
    results = vectorstore.similarity_search("hi", k=1)
    assert len(results) == 1
    
    # Search for a long query (should match the longest document best)
    results = vectorstore.similarity_search("this is a much longer query string", k=1)
    assert len(results) == 1


def test_faiss_vectorstore_custom_index():
    """Test FAISS with a custom index."""
    index = faiss.IndexFlatL2(4)
    embeddings = SimpleEmbeddings()
    vectorstore = FaissVectorStore(embeddings=embeddings, dimension=4, index=index)
